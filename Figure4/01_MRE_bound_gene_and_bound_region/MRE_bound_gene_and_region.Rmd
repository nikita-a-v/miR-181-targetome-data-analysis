...
p <- ggplot(gene_type_df, aes(y=Freq, x="", fill=Var1)) +
     ...
     coord_polar(theta="y") +
  #    xlim(c(2, 4)) +
  geom_label(data = gene_type_df, aes(y=Freq, x="", fill=Var1, label = Freq),
...
ggsave(p, filename = paste0(out, "Figure4d_bound_gene_regions_miR181_BS.pdf"), width = unit(6, "cm"), height = unit(6,"cm"))
...
# Number of MREs per transcript

```{r}
df <- table(mir181_bs$geneID) %>%
  as.data.frame(.) 

df[df$Freq > 10,]$Freq <- 10

ggplot(df, aes(x = Freq))+
  geom_bar()+
  theme_paper()

ggsave(p, filename = paste0(out, "Figure4b_MREs_per_transcript.pdf"), width = unit(6, "cm"), height = unit(6,"cm"))
