...
dir
```{r dir}
setwd("C:/Users/nikit/Documents/Krueger_Lab/Publications/miR181_paper/Figure4/05_RF_MMSAT4")
```
...
# Get repeat Masker annotation for mouse (for metagene plot)
ah = AnnotationHub()
repeat_masker <- ah[["AH99012"]]
repMMSAT4 <- repeat_masker[repeat_masker$repName == "MMSAT4"]
repMurSatRep1 <- repeat_masker[repeat_masker$repName == "MurSatRep1"]

#repeats by transcript (for ECDF plots)
...
```{r metageneplot}
repMMSAT4$id <- "MMsat4"
repMurSatRep1$id <- "MurSatRep1"

metaob <- c(repMMSAT4, repMurSatRep1)

metael <- metaGeneProfile(object = metaob, annotation = gff23path, group = "id")
metasat4 <- metaGeneProfile(object = repMMSAT4, annotation = gff23path)
metamur <- metaGeneProfile(object = repMurSatRep1, annotation = gff23path)
