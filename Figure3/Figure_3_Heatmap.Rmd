...
#diff clip
# eclipGR <- readRDS("C:/Users/nikit/Documents/Krueger_Lab/miReCLIP/Mirco/DifferentialBinding/BsDifferentialResult.rds")
# eclip <- as.data.frame(eclipGR)
# tframe <- eclip[eclip$res.log2FoldChange < 0 & eclip$res.padj <= 0.05,] 
# head(tframe)

#newdiffclip
mir181bs <- readRDS("C:/Users/nikit/Documents/Krueger_Lab/Publications/miR181_paper/Figure1/mir181_binding_sites__venn_types/mir181_bs.rds")
...
plot
```{r plot_kheatmap, fig.dim = c(4, 7)}

kmcent <- 5

set.seed(6)
HRNAk <- Heatmap(kheatframe, show_row_dend = F, show_column_dend = F, show_row_names = F, show_column_names = F, colorRamp2(c(-1, 0, 1), c("#05fae6", "black", "#fa8c05")), column_order=1:12 , bottom_annotation =c(ha3), column_split = c(rep("RNA",6), rep("RPF",6)), row_km = kmcent,
...
```{r sepclust}
set.seed(6)
heat_ks <- kmeans(kheatframe, centers = kmcent)
heat_k_names <- as.data.frame(heat_ks$cluster)
...
tecplot <- ggplot(cTEframe, aes(x=as.factor(cluster), y=log2FoldChange)) + 
  ...
  stat_compare_means(label = "p.format", method = "t.test", ref.group = ".all.", label.y=1)   +                   # Pairwise comparison against all
  theme_paper()
...
#export
pdf("C:/Users/nikit/Documents/Krueger_Lab/Publications/miR181_paper/Figure3/TEbox_clustered.pdf", height = 2, width = 2)
tecplot
dev.off()

pdf("C:/Users/nikit/Documents/Krueger_Lab/Publications/miR181_paper/Figure3/TEhist_clustered.pdf", height = 2, width = 2)
techist
dev.off()
```
