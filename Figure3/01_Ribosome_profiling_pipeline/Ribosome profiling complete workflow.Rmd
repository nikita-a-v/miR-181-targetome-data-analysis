...
```{r dir}
setwd("C:/Users/nikit/Documents/Krueger_Lab/Publications/miR181_paper/Figure3/01_Ribosome_profiling_pipeline")
```
...
You can get these countmatrices from GEO 
```{r dataimport}
#RNA
KOtotal1411 <- read.table("C:/Users/nikit/Documents/Krueger_Lab/Ribo_profiling/run15112022M23/Htseq/clean_FBKOtotal1411_S12_R1_001Aligned.txt")
KOtotal1601 <- read.table("C:/Users/nikit/Documents/Krueger_Lab/Ribo_profiling/run15112022M23/Htseq/clean_FBKOtotal1601_S4_R1_001Aligned.txt")
KOtotal1710 <- read.table("C:/Users/nikit/Documents/Krueger_Lab/Ribo_profiling/run15112022M23/Htseq/clean_FBKOtotal1710_S2_R1_001Aligned.txt")
WTtotal1411 <- read.table("C:/Users/nikit/Documents/Krueger_Lab/Ribo_profiling/run15112022M23/Htseq/clean_FBWTtotal1411_S11_R1_001Aligned.txt")
WTtotal1601 <- read.table("C:/Users/nikit/Documents/Krueger_Lab/Ribo_profiling/run15112022M23/Htseq/clean_FBWTtotal1601_S8_R1_001Aligned.txt")
WTtotal1710 <- read.table("C:/Users/nikit/Documents/Krueger_Lab/Ribo_profiling/run15112022M23/Htseq/clean_FBWTtotal1710_S1_R1_001Aligned.txt")

#RPF
KORPF1411 <- read.table("C:/Users/nikit/Documents/Krueger_Lab/Ribo_profiling/run15112022M23/Htseq/clean_FBKORFP1411_S5_R1_001Aligned.txt")
KORPF1601 <- read.table("C:/Users/nikit/Documents/Krueger_Lab/Ribo_profiling/run15112022M23/Htseq/clean_FBKORFP1601_S7_R1_001Aligned.txt")
KORPF1710 <- read.table("C:/Users/nikit/Documents/Krueger_Lab/Ribo_profiling/run15112022M23/Htseq/clean_FBKORFP1710_S9_R1_001Aligned.txt")
WTRPF1411 <- read.table("C:/Users/nikit/Documents/Krueger_Lab/Ribo_profiling/run15112022M23/Htseq/clean_FBWTRFP1411_S10_R1_001Aligned.txt")
WTRPF1601 <- read.table("C:/Users/nikit/Documents/Krueger_Lab/Ribo_profiling/run15112022M23/Htseq/clean_FBWTRFP1601_S3_R1_001Aligned.txt")
WTRPF1710 <- read.table("C:/Users/nikit/Documents/Krueger_Lab/Ribo_profiling/run15112022M23/Htseq/clean_FBWTRFP1710_S6_R1_001Aligned.txt")
...
Adjust with CombatSeq to reduce batch effects
```{r combatseq}
...
write adjusted count matrices (for possible later use)
```{r writeadjtable}
...
make MA plots for adjusted RNAseq and Ribosome Footprint
...
### import targets
import targeting data from previous figures to be able to mark miR-181 targets in MA plots
```{r targets}
#targets
...
RNAma <- ggplot(RNAdat[!(RNAdat$xcutoff == "high"),], aes(y=log2FoldChange, x=log10(baseMean))) + geom_point_rast(shape=21, size = 0.5, 
...
pdf("MAplot_RNA.pdf", width = 1.5, height = 1.8)
RNAma
dev.off()
```
RPF MA plot
```{r RPFMA, fig.width=2.5, fig.height=2.5}
RPFma <- ggplot(RPFdat[!(RPFdat$xcutoff == "high"),], aes(y=log2FoldChange, x=log10(baseMean))) + geom_point_rast(shape=21, size = 0.5, 
     aes(fill = factor(LFCandPADJSig), colour=factor(LFCandPADJSig))) +
  scale_fill_manual(values = c(negcol, RNAncol, RNApcol)) + scale_colour_manual(values = c(negcol, "black", "black")) +
...
pdf("MAplot_RPF.pdf", width = 1.5, height = 1.8)
RPFma
dev.off()
```

# Volcano plots for supplementary
...
#make columns with identifiers
RNAdat$ycutoff[RNAdat$padj < plim] <- "phigh"

#implement cutoff
RNAdat$padj[RNAdat$padj < plim] <- plim
RNAdat$baseMean[RNAdat$baseMean > xlim] <- xlim

table(RNAdat$ycutoff)

RNAvol <- ggplot(RNAdat, aes(y=-log10(padj), x=log2FoldChange)) + geom_point_rast(shape=21, size = 0.5, 
     aes(fill = factor(LFCandPADJSig), colour=factor(LFCandPADJSig))) +
  scale_fill_manual(values = c(negcol, RNAncol, RNApcol)) + scale_colour_manual(values = c(negcol, "black", "black")) +
  geom_point(data=RNAdat[RNAdat$LFCandPADJSig == "Significant down" & RNAdat$ycutoff =="in" & RNAdat$xcutoff == "in",], aes(y=-log10(padj), x=log2FoldChange), shape=21, size = 1, colour="black", fill=RNAncol) +
  geom_point(data=RNAdat[RNAdat$LFCandPADJSig == "Significant up" & RNAdat$ycutoff =="in" & RNAdat$xcutoff == "in",], aes(y=-log10(padj), x=log2FoldChange), shape=21, size = 1, colour="black", fill=RNApcol) +
  geom_point(data=RNAdat[RNAdat$LFCandPADJSig == "Significant up" & RNAdat$ycutoff =="phigh" & RNAdat$xcutoff == "in",], aes(y=-log10(padj), x=log2FoldChange), shape=24, size = 1, colour="black", fill=RNApcol) +
  geom_point(data=RNAdat[RNAdat$LFCandPADJSig == "Significant down" & RNAdat$ycutoff =="phigh" & RNAdat$xcutoff == "in",], aes(y=-log10(padj), x=log2FoldChange), shape=24, size = 1, colour="black", fill=RNAncol) +
  theme_paper() +
  geom_hline(yintercept = -log10(pvsig), linetype="solid", colour="black") + 
  geom_vline(xintercept = lfcsig, linetype="dashed", colour="black") + 
  geom_vline(xintercept = -lfcsig, linetype="dashed", colour="black") + 
  scale_y_continuous("Adjusted p-value (-log10)") + scale_x_continuous("Fold change (log2)") +
  guides(color = guide_legend(override.aes = list(size = 3))) + ggtitle("RNAseq")

RNAvol

pdf("Volcanoplot_RNA.pdf", width = 2, height = 2)
RNAvol
dev.off()
```
## RPF
...
```{r RPFvolcano}
#make columns with identifiers
RPFdat$ycutoff[RPFdat$padj < plim] <- "phigh"

#implement cutoff
RPFdat$padj[RPFdat$padj < plim] <- plim
RPFdat$baseMean[RPFdat$baseMean > xlim] <- xlim

table(RPFdat$ycutoff)

RPFvol <- ggplot(RPFdat, aes(y=-log10(padj), x=log2FoldChange)) + geom_point_rast(shape=21, size = 0.5, 
     aes(fill = factor(LFCandPADJSig), colour=factor(LFCandPADJSig))) +
  scale_fill_manual(values = c(negcol, RNAncol, RNApcol)) + scale_colour_manual(values = c(negcol, "black", "black")) +
  geom_point(data=RPFdat[RPFdat$LFCandPADJSig == "Significant down" & RPFdat$ycutoff =="in" & RPFdat$xcutoff == "in",], aes(y=-log10(padj), x=log2FoldChange), shape=21, size = 1, colour="black", fill=RNAncol) +
  geom_point(data=RPFdat[RPFdat$LFCandPADJSig == "Significant up" & RPFdat$ycutoff =="in" & RPFdat$xcutoff == "in",], aes(y=-log10(padj), x=log2FoldChange), shape=21, size = 1, colour="black", fill=RNApcol) +
  geom_point(data=RPFdat[RPFdat$LFCandPADJSig == "Significant up" & RPFdat$ycutoff =="phigh" & RPFdat$xcutoff == "in",], aes(y=-log10(padj), x=log2FoldChange), shape=24, size = 1, colour="black", fill=RNApcol) +
  geom_point(data=RPFdat[RPFdat$LFCandPADJSig == "Significant down" & RPFdat$ycutoff =="phigh" & RPFdat$xcutoff == "in",], aes(y=-log10(padj), x=log2FoldChange), shape=24, size = 1, colour="black", fill=RNAncol) +
  theme_paper() +
  geom_hline(yintercept = -log10(pvsig), linetype="solid", colour="black") + 
  geom_vline(xintercept = lfcsig, linetype="dashed", colour="black") + 
  geom_vline(xintercept = -lfcsig, linetype="dashed", colour="black") + 
  scale_y_continuous("Adjusted p-value (-log10)") + scale_x_continuous("Fold change (log2)") +
  guides(color = guide_legend(override.aes = list(size = 3))) + ggtitle("RPF")

RPFvol

pdf("Volcanoplot_RPF.pdf", width = 2, height = 2)
RPFvol
dev.off()
```
...
RNAexpnolim <- resdataadj

RPFexpnolim <- resdataRPFadj
...
#no cutoff
genesnolim <- RNAexpnolim$Gene
G_listnolim <- getBM(filters= "ensembl_gene_id_version", attributes= c("ensembl_gene_id_version","mgi_symbol"),values=genesnolim,mart= mart)
colnames(G_listnolim) <- c("Gene", "gene_symbol")
RNAexpnolim <- left_join(RNAexpnolim,G_listnolim,by="Gene")
RPFexpnolim <- merge(RPFexpnolim,G_listnolim,by="Gene")
head(RNAexpnolim)
head(RPFexpnolim)
...
write.csv(RNAexpnolim, "RNA_masterframe_nocutoff.csv")
write.csv(RPFexpnolim, "RPF_masterframe_nocutoff.csv")

# and as xlsx for supplementary
write_xlsx(RNAexp, "RNA_masterframe.xlsx")
write_xlsx(RPFexp, "RPF_masterframe.xlsx")
