...
#export
pdf("C:/Users/nikit/Documents/Krueger_Lab/Publications/miR181_paper/Figure3/RPFandRNA_tbar.pdf", width = 1, height = 2)
ribtbarp
dev.off()
...
table(msco$significance)

```

## MS Barplots
```{r MSbarplots}
mscobardat <- data.frame(c("Significantly_up", "Significantly_down"),
                        c(length(msco[msco$log2FoldChange > 0 & msco$padj <= 0.05 & !is.na(msco$padj),"gene_symbol"]), -length(msco[msco$log2FoldChange < 0 & msco$padj <= 0.05 & !is.na(msco$padj),"gene_symbol"])))
colnames(mscobardat)=c("ID", "Number_of_genes")


mscobar <- ggplot(data=mscobardat, aes(x=ID, y=Number_of_genes, fill=ID)) +
...
```{r MStargets}
#RNA
msco$target <- "Non-target"
msco$target[msco$gene_symbol %in% largetframe$geneName] <- "Target"

MStbaru <- as.data.frame(table(msco[msco$padj <= 0.05 & msco$log2FoldChange > 0, "target"]))
MStbaru$y <- "Up"
MStbaru$Freq <- MStbaru$Freq/sum(MStbaru$Freq)

MStbard <- as.data.frame(table(msco[msco$padj <= 0.05 & msco$log2FoldChange < 0, "target"]))
MStbard$y <- "Down"
MStbard$Freq <- MStbard$Freq/sum(MStbard$Freq)

#combine
MSbar <- rbind(MStbaru,MStbard)

MStbarp <- ggplot(MSbar, aes(x=y, y=Freq, fill=Var1)) + geom_bar(stat = "identity", position = "stack") +
  theme_paper()

MStbarp

#export
pdf("C:/Users/nikit/Documents/Krueger_Lab/Publications/miR181_paper/Figure3/MS_tbar.pdf", width = 1, height = 2)
MStbarp
dev.off()
...
```{r tetargets}
#RNA
TEco$target <- "Non-target"
TEco$target[sub("\\..*", "", TEco$Gene) %in% largetframe$geneID] <- "Target"

TEtbaru <- as.data.frame(table(TEco[TEco$padj <= 0.05 & TEco$log2FoldChange > 0, "target"]))
TEtbaru$y <- "Up"
TEtbaru$Freq <- TEtbaru$Freq/sum(TEtbaru$Freq)

TEtbard <- as.data.frame(table(TEco[TEco$padj <= 0.05 & TEco$log2FoldChange < 0, "target"]))
TEtbard$y <- "Down"
TEtbard$Freq <- TEtbard$Freq/sum(TEtbard$Freq)

#combine
TEbar <- rbind(TEtbaru,TEtbard)

TEtbarp <- ggplot(TEbar, aes(x=y, y=Freq, fill=Var1)) + geom_bar(stat = "identity", position = "stack") +
  theme_paper()

TEtbarp

#export
pdf("C:/Users/nikit/Documents/Krueger_Lab/Publications/miR181_paper/Figure3/TE_tbar.pdf", width = 1, height = 2)
TEtbarp
dev.off()
...
# RPF vs MS
```{r RPFvsMS}
names(ms)[2] <- "MS_log2FoldChange"

msribo <- inner_join(RPF[!is.na(RPF$RPF_log2FoldChange),], ms[!is.na(ms$MS_log2FoldChange),], by="gene_symbol")
