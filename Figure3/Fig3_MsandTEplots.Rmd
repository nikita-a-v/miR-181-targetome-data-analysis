...
riboplot <- ggplot(ribo, aes(x=RNA_log2FoldChange, y=RPF_log2FoldChange, colour=density)) +
  ...
  scale_colour_gradientn(colours = c(farbe3, farbe3, farbeneg), values = c(7, 0.25, 0)) +
  theme_paper()
...
#plot
msriboplot <- ggplot(msribo, aes(x=MS_log2FoldChange, y=RPF_log2FoldChange, fill=density)) +
  geom_point_rast(size=1, shape=21, colour="black", stroke=0.1) +
  geom_abline(slope = 1, colour=farbe1) +
  geom_hline(yintercept = 0, linetype="dashed") +
  geom_vline(xintercept = 0, linetype="dashed") +
  coord_cartesian(ylim = c(-3,3), xlim = c(-3,3)) +
  scale_fill_gradientn(colours = c(farbe3, farbe3, farbeneg), values = c(7, 0.25, 0)) +
  theme_paper()
