...
bs_wt <- readRDS("/Users/melinaklostermann/Documents/projects/AgoCLIP_miR181/R_github/miR181_paper/Figure1/AGO_binding_site_definition/2023-04-21-AGO_BS.rds")
...
p <- ggplot(df, aes(x = rank, y = value)) +
    ...
    guides(color = guide_colorbar(
                          barwidth = unit(0.25, "cm"), barheight = unit(3, "cm"))) +
    labs(
        x = "Rank",
        y = "TPMs",
        color = "#Points"
...
ggsave(p, filename = paste0(out, "SupF1_tpms_diff_binding.pdf"), height = 12, width = 12, units = "cm")
...
pdf(file=paste0(out, "SupF1_diff_binding_set.pdf"))
ht = UpSet(m,
           comb_order = order(comb_size(m), decreasing = T),
           top_annotation = ha,
           right_annotation = ha2,
...
ggsave(p1, filename = "/Users/melinaklostermann/Documents/projects/AgoCLIP_miR181/R_github/miR181_paper/Figure1/Differential_Binding/S1L_differential_regionwise.pdf", height = 6, width = 6, units = "cm")
...
dfLabMa_up = df %>% 
  filter(geneName %in%  selectedGenes & resBs.log2FoldChange > 0 & sig == TRUE)
...
write.csv(diff_bs, paste0(out, "supplementary_table_differentail_binding.csv"))

diff_bs %>% subset(resBs.padj < 0.05) %>% subset(gene)
