...
out <- paste0(here,"/Figure6/01_RNAduplex/")
...
ggsave(p, filename = paste0(out, "SuppFigure6A_MRE_with_seed_bar.pdf"), width = 6, height = 6, units = "cm")
```
...
struct <- read_table(paste0(out,"/mir181_enriched_set_237nt.struct"), col_names = c("seq", "mir", "structure", "position_seq", "x", "position_mir", "min_free_energy"), col_types = "ccccccc")
...
ggsave(p1, filename = paste0(out, "SuppFigure6C_duplex_start_position.pdf"), width = 6, height = 4, units = "cm")
...
ggsave(p2, filename = paste0(out, "SuppFigure6C_canonical_duplex_seeds_bar.pdf"), width = 6, height = 6, units = "cm")
...
k2 <- kmeans(mat_can_seed, centers = K)
can_seed$kmeans2 <- k2$cluster
...
Heatmap(mat_can_seed, cluster_rows = F, cluster_columns = F, col = col_fun, split = k2$cluster, right_annotation = ra)
...
pdf(file = paste0(out,"ReviewerQ12_HM_with_seed_k=", K,".pdf"))
Heatmap(mat_can_seed, cluster_rows = F, cluster_columns = F, col = col_fun, split = k2$cluster, right_annotation = ra,  use_raster = T, raster_device = "png", raster_quality = 10)
dev.off()
```
...
ggsave( filename = paste0(out, "SuppFigureH_pureclip_score_per_cluster_can_k=", K, ".pdf"), width = 6, height = 5, units = "cm")
...
ggsave( filename = paste0(out, "FigureS6E_min_free_energy_per_cluster_can_k=", K, ".pdf"), width = 6, height = 5, units = "cm")
...
ggsave(p2, filename = paste0(out, "SuppFigure6B_no_seed_seed_binding_bar.pdf"), width = 6, height = 6, units = "cm")
```
...
## Heamap with parital binding in first 7 nt
...
ggsave( filename = paste0(out, "SuppFigure6I_pureclip_score_per_cluster_noncan_noseed_k=", K, ".pdf"), width = 6, height = 5, units = "cm")
...
ggsave( filename = paste0(out, "SuppFigureS6J_pureclip_score_per_cluster_noncan_partseed_k=", K, ".pdf"), width = 6, height = 5, units = "cm")
...
ggsave( filename = paste0(out, "SuppFigureS6F_min_free_energy_per_cluster_noncan_noseed_k=", K, ".pdf"), width = 6, height = 5, units = "cm")
...
ggsave( filename = paste0(out, "SuppFigureS6G_min_free_energy_per_cluster_noncan_partseed_k=", K, ".pdf"), width = 6, height = 5, units = "cm")
...
ggsave( filename = paste0(out, "SuppFigureS6d_ecdf_canonical_vs_non_canonical_duplex.pdf"), width = 5, height = 6, units = "cm")
...
ggsave( filename = paste0(out, "Figure6B_ecdf_partil_vs_no_seed_binding.pdf"), width = 5, height = 6, units = "cm")
```
...
rfp_can <- left_join(rfp, 
                     can_seed %>% dplyr::select("geneID", "kmeans"), 
                     by = c(Gene = "geneID"))  %>% 
...
ggsave( filename = paste0(out, "Figure6d_ecdf_clusters_can_k=", K, ".pdf"), width = 5, height = 8, units = "cm")
...
ggsave( filename = paste0(out, "Figure6H_ecdf_clusters_nocan_noseed_k=", K, ".pdf"), width = 9, height = 5, units = "cm")
...
ggsave( filename = paste0(out, "Figure6H_ecdf_clusters_nocan_noseed_zoomIn_k=", K, ".pdf"), width = 9, height = 5, units = "cm")
...
ggsave( filename = paste0(out, "Figure6F_ecdf_clusters_nocan_partseed_k=", K, ".pdf"), width = 7, height = 5, units = "cm")
...
ggsave( filename = paste0(out, "Figure6F_ecdf_clusters_nocan_partseed_zoomIns_k=", K, ".pdf"), width = 7, height = 5, units = "cm")
...
## MMSAT4
```{r}
mat_mmsat4 <- mmsat4_st %>% dplyr::select(V2:V24) %>%
  as.matrix()
...
## MurSatRep1
```{r}
mat_mursatrep1 <- mursatrep1_st %>% dplyr::select(V2:V24) %>%
  as.matrix()
