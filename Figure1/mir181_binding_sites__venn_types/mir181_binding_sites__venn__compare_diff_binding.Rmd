...
```{r nikitasviolin}
mycomp <- list(c("ago_bs_mir181_chi", "ago_bs_mir181_chi&mir181_enriched"), c("ago_bs_mir181_chi&mir181_enriched", "mir181_enriched"), c("ago_bs_mir181_chi", "mir181_enriched"))
table(mir181_bs_diff$set)

p2n <- ggplot(mir181_bs_diff[!is.na(mir181_bs_diff$set),], aes(y = resBs.log2FoldChange, x = as.character(set), fill = as.character(set)))+
  geom_half_violin()+
  geom_half_boxplot(side = "r")+
  geom_hline(yintercept = 0, linetype="dashed") +
  theme_paper()+
  scale_fill_manual(values = c(farbe1, farbe14, farbe2))+
  theme(legend.position = "top")+
  scale_x_discrete(guide = guide_axis(angle = 25)) +
  stat_compare_means(comparisons = mycomp)
  
p2n
ggsave(p2n, filename =  paste0(out, "Figure_1J_violin_differntial_binding_vs_mir181BS_pvalues.pdf"), width = 4, height = 7, units = "cm")
