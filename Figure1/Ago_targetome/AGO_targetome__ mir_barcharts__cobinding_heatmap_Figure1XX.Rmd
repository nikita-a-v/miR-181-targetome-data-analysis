...
p4 <- ggplot(mir_imgen, aes(x = log(mean), y = log(Freq)))+
  ...
  geom_point(data=mir_imgen[mir_imgen$ID_REF == "miR-181a-5p",],aes(x = log(mean), y = log(Freq)), fill="#8D0391", colour="black", size=4, shape=21)+ # changed by nikita
  geom_point(data=mir_imgen[mir_imgen$ID_REF == "miR-181b-5p",],aes(x = log(mean), y = log(Freq)), fill="#8D0391", colour="black", size=4, shape=21)+ # changed by nikita
  geom_text_repel(data=mir_imgen[mir_imgen$ID_REF %in% c("miR-181a-5p", "miR-181b-5p"),], aes(x = log(mean), y = log(Freq), label=ID_REF))+ #Nikita
