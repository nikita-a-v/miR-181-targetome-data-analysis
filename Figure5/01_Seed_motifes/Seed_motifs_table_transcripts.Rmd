...
p <- ggplot(Seeds_500down %>% subset(seed = "seed_6mer"), aes(x = start))+
      geom_rect(inherit.aes=FALSE, aes(xmin=500, xmax=700, ymin = 0, ymax = 250), color="transparent", fill="orange", alpha=0.01)+
...
ggplot(Seeds_500down %>% subset(seed = "seed_6mer"), aes(x = start))+
      geom_rect(inherit.aes=FALSE, aes(xmin=500, xmax=700, ymin = 0, ymax = 600), color="transparent", fill="orange", alpha=0.01)+
  geom_histogram(binwidth = 5)+
  theme_bw()+
  scale_x_continuous(breaks = c(100,300, 500, 700, 900), labels = c("-400", "-200", "0", "200", "400"))

ggplot(Seeds_500down %>% subset(seed = "seed_6mer"), aes(x = start))+
      geom_rect(inherit.aes=FALSE, aes(xmin=400, xmax=600, ymin = 0, ymax = 600), color="transparent", fill="lightblue", alpha=0.01)+
  geom_histogram(binwidth = 5)+
  theme_paper()+
  scale_x_continuous(breaks = c(100,300, 500, 700, 900), labels = c("-400", "-200", "0", "200", "400"))

ggsave(p, filename = paste0(out, "Revision_seed_around_bs.pdf"), height = 6, width = 8, units ="cm")

```

## Compare RFP 50nt to 200nt
...
df_plot <- mir181_bs %>% 
  mutate(seed_dist_group = case_when(first_seed_200down.start <= 50 ~ "0-50",
                                     first_seed_200down.start > 50 ~ "150-200",
                                     T ~ "No seed in window")) %>%
  group_by(geneID) %>%
  arrange(scoreMean, .by_group = T) %>%
  slice(1)

df_plot <- df_plot %>% left_join(rfp, by = c(geneID = "Gene" ))


ggplot(df_plot, aes(x = log2FoldChange, color = seed_dist_group))+
  geom_vline(xintercept = 0)+
  stat_ecdf()+
  coord_cartesian(xlim = c(-0.5, 0.5))+
  theme_paper()

ggsave( filename = paste0(out, "Revision_rfp_50_200_seed_window.pdf"), height = 6, width = 8, units ="cm")
